...
func FindOrderListWithDate(date time.Time) (orders []Order) {
	Db.Raw("select * from orders where created_at >= date('%s') and created_at < date('%s', '+1 day')", date.Format("2006-01-02"), date.Format("2006-01-02")).Scan(&orders)
	return orders
}

func FindOrderListWithMonth(date time.Time) (orders []Order) {
	Db.Raw("select * from orders where created_at >= date('%s') and created_at < date('%s', '+1 month')", date.Format("2006-01-02"), date.Format("2006-01-02")).Scan(&orders)
